...
class RiskHistoricalDataServiceConnector : public Connector<PV01<T>>
{
	...
	public:
		virtual void Publish(PV01<T>& data); // publish PV01 data
		virtual void Publish(PV01<T>& data, std::vector<PV01<BucketedSector<T>>> sectors); // publish PV01 and bucketed sectors
		virtual void Subscribe(std::string filePath);
};
...
void RiskHistoricalDataServiceConnector<T>::Publish(PV01<T>& data, std::vector<PV01<BucketedSector<T>>> sectorsPV01)
{
	...
	for (const auto& pv01 : sectorsPV01) // record the sector risks as well
	{
		file << "{" << pv01.str() << "}";
	}
	file << std::endl;
...
void ExecutionHistoricalDataServiceConnector<T>::Publish(ExecutionOrder<T>& data)
{
	...
	ptime now = microsec_clock::local_time();
	file = std::ofstream(filePath, std::ofstream::out | std::ofstream::app); // append to original text
	file << now << "," << data.str() << std::endl;
	file.close();
}
